...
N = 256
dists = np.logspace(np.log10(30),np.log10(0.1),N)
params = []
for i in list(range(N)):
    params.append({"Time": 3e-3,
                   "camera_distance": dists[i],
...
cubemap_params = []
for p in params:
    cubemap_params += cubemapify(p)
params = [cubemap_params] # format as a list of lists with one entry per task
Nchunks = 16 
