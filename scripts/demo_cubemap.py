...
centers = center + np.zeros((tot_frames,3)) #np.c_[8*np.cos(3*np.pi*np.arange(tot_frames)/tot_frames), np.zeros(tot_frames),8*np.sin(6*np.pi*np.arange(tot_frames)/tot_frames)]
