    ...
    for i in task_chunk_indices:        
        ...
        if task_params[0][i]["sparse_snaps"]: 
            if np.any(time==snaptimes):
                ...
                time += 1e-6 * np.ptp(snaptimes) #makes sure that time does not exactly match snapshot times, forcing the use of SnapInterpolate
        ...
        if time in snapdata_buffer.keys(): # if we have the data for this exact time in the buffer, no action needed
            ...
            if t1 != t2:
                print("%d: Interpolating data for time %g from snapshots at times %g and %g ..."%(process_num, time, t1, t2))
                snapdata_for_thistime = SnapInterpolate(time, t1, t2, snapdata_buffer, sparse_snaps=task_params[0][i]["sparse_snaps"])
            else:
                print("%d: Interpolating data is not possible for time %g, using data from snapshot at time %g ..."%(process_num, time, t1))
...
def SnapInterpolate(t,t1,t2,snapdata_buffer, sparse_snaps=False):
    ...
    stuff_to_leave_as_is = [] #keep things unchanged but still include them in snapdata_buffer, used mostly for debugging
    stuff_to_keep_lowest = [] #keep the lowest value between snapshots, can be used to remove spikes that exist for only one snapshot that are hard to interpolate
    
    interpolated_data = snapdata_buffer[t1].copy()
    idx1, idx2 = {}, {}
    for ptype in "PartType0","PartType5":
        if ptype+"/ParticleIDs" in snapdata_buffer[t1].keys(): id1 = np.array(snapdata_buffer[t1][ptype+"/ParticleIDs"])
        else: id1 = np.array([])
        if ptype+"/ParticleIDs" in snapdata_buffer[t2].keys(): id2 = np.array(snapdata_buffer[t2][ptype+"/ParticleIDs"])
        else: id2 = np.array([])
    ...
    for field in snapdata_buffer[t1].keys():
        if not (field in stuff_to_skip):
            ...
            wt1 = (t2 - t)/(t2 - t1)* np.ones_like(f1) #relative weights, can be set individually for each cell, for now we just use the same time linear weight for all cells
            wt2 = 1.0 - wt1
            if (field in stuff_to_leave_as_is):
                ...
                if field in stuff_to_interp_log:        
                    ...
                    if np.any(~positive_ind): #we interpolate linearily for cells where the value in either snapashots is non-positive, log for the rest
                        interpolated_data[field] = f1 * wt1 + f2 * wt2 
                        interpolated_data[field][positive_ind] = np.exp(np.log(f1[positive_ind]) * wt1[positive_ind] + np.log(f2[positive_ind]) * wt2[positive_ind])
                    else:
