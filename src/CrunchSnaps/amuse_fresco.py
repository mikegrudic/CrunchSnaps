...
def lum_MS(m):
    ...
    lum = (0.39704170 * (m**5.5) + 8.52762600 * (m**11)) / (
        0.00025546
        + (m**3)
        + 5.43288900 * (m**5)
        + 5.56357900 * (m**7)
...
def rad_MS(m):
    ...
    rad = (
        ...
        + 1.01249500 * (m**19)
        + 0.07490166 * (m**19.5)
    ) / (0.01077422 + 3.08223400 * (m**2) + 17.84778000 * (m**8.5) + (m**18.5) + 0.00022582 * (m**19.5))
    return rad
...
def optical_depth_for_targets(target_coords, x_obs, x, m, h, kappa, nthreads=1):
    ...
    dx = x - x_obs
    target_coords = target_coords - x_obs
    ##################
    ...
    target_coords_fake = np.vstack(
        (target_coords_sph[:, 1], target_coords_sph[:, 2], np.zeros(target_coords_sph.shape[0]))
    ).T
    ind_all = tree3d.query_ball_point(
        target_coords_fake, K, workers=nthreads
    )  # particles within distance K in the fake 3D space will be within their actual radius in the 2D space
    ...
    for i, ind in enumerate(ind_all):
        ...
        optical_depths[i] = np.sum(
            1.8189136353359467 * kernel * d_tau[ind][between_ind]
        )  # This means we take either 100% or zero of a gas cell's d_tau, with a hard transition at the softening length
    return optical_depths


def make_amuse_fresco_stars_only(
    x,
    mstar,
    age_yr,
    L,
    ...
    if (mass_limits[0] != 0.0) or (mass_limits[1] != 0.0):
        if mass_limits[0] == 0.0:
            mass_limits[0] = np.min(mstar)
        if mass_limits[1] == 0.0:
            mass_limits[1] = np.max(mstar)
    ...
    # from_se.copy()
    # inspectvar(stars)
    image, _ = make_fresco_image(
        stars,
        gas,
        return_vmax=True,
        image_width=[L | units.pc, L | units.pc],
        image_size=[res, res],
        percentile=1 - p,
        vmax=vmax,
    )
